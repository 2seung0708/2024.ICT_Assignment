...
class Evaluator(object):
    ...
    def get_accuracy(self, cfg, epoch, n_test, testset_name, n_min=4, thres=1, logger=None):
        ...
        for idx, out_dict in tqdm(zip(self.img_ids_all, self.outputs_all)):
            ...
            n_correct1, _, ticks1 = utils.ADD_accuracy_withID_debug(cfg, P1, pts_model_h, PM_gt, diameter,save_name="vis_3d_plot_{}.png".format(idx))     
            ...
            # ######### code for visualize keypoints
            # import pdb;pdb.set_trace()
            # img, img_path = utils.get_img_trp(cfg,idx,self.test_mask)
            
            # img = torch.tensor(img).permute(2,0,1)
            
            # pts2d_gt = utils.batch_project(PM_gt,pts3d_fps,K,angle_axis=False).squeeze() ## fps 128 시각화
            # # pts2d_gt = utils.batch_project(PM_gt,pts_model_h[:,:3],K,angle_axis=False).squeeze() ## 3d model 시각화
            # vis_gt = kp_vis(pts2d_gt, img)
            # vis1 = kp_vis(pts2d_out_coord1.squeeze(), vis_gt, color=(0, 0, 255))
            # vis2 = kp_vis(pts2d_out_coord2.squeeze(), vis_gt, color=(0, 0, 255))
            # vis3 = kp_vis(pts2d_out_coord3.squeeze(), vis_gt, color=(0, 0, 255))
            # res = np.hstack([vis1,vis2,vis3])
            
            # import pdb;pdb.set_trace()
            
            # save_p=f'{cfg.OUTPUT_DIR}/{cfg.obj}/{cfg.log_name}_Vis/keypoints/'
            # os.makedirs(save_p, exist_ok=True)
            
            # save_name="__".join([cfg.TEST_SEQ]+[x for x in img_path.replace(cfg.TRP_ORI_DIR ,"").split("/") if (("seq_test" in x) or ("png" in x))])
            

            # cv.imwrite(f"{save_p}/{save_name}", res)
            # ####################################
...
def pnp(pts2d, pts3d, K):
    ...
    for i in range(bs):
        ...
        if T[0,2] < 0:
            RR = kn.geometry.angle_axis_to_rotation_matrix(angle_axis)
            RR = R_inv.matmul(RR)
