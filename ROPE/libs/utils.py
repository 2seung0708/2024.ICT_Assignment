...
LINEMOD_LIST_P="/home/shlee/workspace/6D_PoseEstimation/dataset/ffb6d_preprocessed/Linemod_preprocessed/data"
...
def get_lm_img_idx(cfg, n_lm, n_lm_synt=None):
    ...
    f = open(os.path.join(LINEMOD_LIST_P,'{:02d}/train.txt'.format(objid))) ## test_list in BOP challenge
    # f = open(os.path.join(LINEMOD_LIST_P,cfg.obj.lower(),'training_range.txt'))
...
def get_lm_o_fps(cfg):
    ...
    objid = get_objid(cfg.obj)
    fps_file = './dataset/fps/lm/obj{:02d}_fps128.mat'.format(objid)
...
def get_lm_o_3dmodel(cfg, homo=False):
    objid = get_objid(cfg.obj)
    model_file = './dataset/obj{:02d}.mat'.format(objid)
...
def get_distance(cfg):
    ...
    objid = get_objid(cfg.obj)
    with open(os.path.join(cfg.LM_DIR,'../models/models/models_info.json')) as f:
...
def ADD_accuracy_withID_debug(cfg, P, pts3d_h, PM_gt, diameter, P_is_matrix=False,save_name="None"):
    bs = P.size(0)
    if P_is_matrix:
        PM = P
    else:
        # R_out = kn.angle_axis_to_rotation_matrix(P[:, 0:3].view(bs, 3)) ## original
        R_out = kn.geometry.angle_axis_to_rotation_matrix(P[:, 0:3].view(bs, 3)) ## shlee
        PM = torch.cat((R_out[:, 0:3, 0:3], P[:, 3:6].view(bs, 3, 1)), dim=-1)## original
        # PM = torch.cat((R_out[:, 0:3, 0:3], P[:, 3:6].view(bs, 3, 1)/10), dim=-1)
    img = np.zeros((cfg.MODEL.IMAGE_SIZE[1],cfg.MODEL.IMAGE_SIZE[0],3))
    pts3d_cam = pts3d_h.matmul(PM.transpose(1, 2))
    pts3d_cam_gt = pts3d_h.matmul(PM_gt.transpose(1, 2))
    ...
    # # ax1.set_title("GT")
    # # ax1.scatter(pts3d_cam_gt.squeeze()[:,0][::10],pts3d_cam_gt.squeeze()[:,1][::10],pts3d_cam_gt.squeeze()[:,2][::10],color="r") ## GT
    # # ax2 = fig.add_subplot(122, projection='3d')
    # # ax2.set_title("Predict")
    # # ax2.scatter(pts3d_cam.squeeze()[:,0][::10],pts3d_cam.squeeze()[:,1][::10],pts3d_cam.squeeze()[:,2][::10],color="g") ## Pred
    
    # ax1 = fig.add_subplot(111, projection='3d')
    # ax1.scatter(pts3d_cam_gt.squeeze()[:,0][::10],pts3d_cam_gt.squeeze()[:,1][::10],pts3d_cam_gt.squeeze()[:,2][::10],color="r") ## GT
    # ax1.set_label("GT")
    # ax1.scatter(pts3d_cam.squeeze()[:,0][::10],pts3d_cam.squeeze()[:,1][::10],pts3d_cam.squeeze()[:,2][::10],color="g") ## Pred
    # ax1.set_label("Pred")
    
    # os.makedirs(f'{cfg.OUTPUT_DIR}/{cfg.obj}/{cfg.log_name}_Vis/',exist_ok=True)
    
    # plt.savefig(f'{cfg.OUTPUT_DIR}/{cfg.obj}/{cfg.log_name}_Vis/{save_name}')
...
def ADDS_accuracy(P, pts3d_h, PM_gt, diameter, P_is_matrix=False):
    ...
    if P_is_matrix:
        ...
        R_out = kn.geometry.angle_axis_to_rotation_matrix(P[:, 0:3].view(bs, 3))
        PM = torch.cat((R_out[:, 0:3, 0:3], P[:, 3:6].view(bs, 3, 1)), dim=-1)
    pts3d_cam = pts3d_h.matmul(PM.transpose(1, 2))
    pts3d_cam_gt = pts3d_h.matmul(PM_gt.transpose(1, 2))
