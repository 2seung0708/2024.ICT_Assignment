...
def main(args, cfg):
    ...
    if args.test_only:
        evaluator = evaluate(model, data_loader_test, device=device, logger=logger,cfg=cfg)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--hr_ckpt', required=True, type=str)
    parser.add_argument('--no_use_synt',action="store_true",help="set para if you don't use synthetic")
    args = parser.parse_args()
    cfg = CN(new_allowed=True)
    cfg.defrost()
    cfg.merge_from_file(args.cfg)
    cfg.obj = args.obj
    cfg.MODEL.PRETRAINED=args.hr_ckpt ## shlee added
